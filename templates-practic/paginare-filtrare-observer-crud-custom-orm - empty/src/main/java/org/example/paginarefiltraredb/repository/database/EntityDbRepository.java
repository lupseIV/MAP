...
import org.example.paginarefiltraredb.repository.paging.FilterRepository;
...
public abstract class EntityDbRepository<ID, E extends Entity<ID>> extends EntityRepository<ID, E>
        ...
        try (DatabaseConnection.AutoCloseableConnection conn = DatabaseConnection.getConnection();
            ...
            try (ResultSet resultSet = statement.executeQuery()) {
                while (resultSet.next()) {
                    E dbEntity = extractEntityFromResultSet(resultSet);
                    // Return from memory if exists, otherwise the db version
                    entitiesList.add(super.findOne(dbEntity.getId()).orElse(dbEntity));
                }
            }
            return entitiesList;
        } catch (SQLException e) {
            throw new RepositoryException("Error in filtered findAll", e);
        }
    }
    ...
    public Page<E> findAllOnPage(Pageable pageable, SqlFilter filter) {
        try (DatabaseConnection.AutoCloseableConnection conn = DatabaseConnection.getConnection();) {
            int totalNumberOfTuples = count(conn.get(), filter);
            List<E> tuplesOnPage;
            if (totalNumberOfTuples > 0) {
                tuplesOnPage = findAllOnPage(conn.get(), pageable, filter);
            } else {
