...
public abstract class EntityDbRepository<ID, E extends Entity<ID>>
    ...
    public List<E> findAll(SqlFilter filter) {
        ...
        try (AutoCloseableConnection connection = DatabaseConnection.getConnection();
            ...
            try (ResultSet resultSet = statement.executeQuery()) {
                while (resultSet.next()) {
                    // We extract the entity and return the instance from the cache
                    // if it exists to maintain referential integrity
                    E entity = extractEntityFromResultSet(resultSet);
                    Optional<E> cachedEntity = super.findOne(entity.getId());
                    entitiesList.add(cachedEntity.orElse(entity));
                }
            }

            return entitiesList;
        } catch (SQLException e) {
            throw new RepositoryException("Error finding entities with filter", e);
        }
    }
