...
public class VipClientControllerTableView extends AbstractTableViewController<Integer, VipClient, VipClientDto>
    ...
    private List<String> getManagerOptions() {
        try {
            Iterable<Manager> managers = managerService.findAll().get();
            List<String> options = StreamSupport.stream(managers.spliterator(), false)
                    .map(m -> m.getId() + ": " + m.getName())
                    .collect(Collectors.toCollection(ArrayList::new));
            options.add(0, "Unassigned");
            return options;
        } catch (Exception e) {
            e.printStackTrace();
            return new ArrayList<>(List.of("Unassigned"));
        }
    }

    // --- HELPER: Find Manager by formatted String ---
    private CompletableFuture<Manager> resolveManagerSelection(String selection) {
        ...
        try {
            ...
            long id = Long.parseLong(idPart); // Use Long to match Manager ID type
    ...
    public VipClient createEntity(Map<String, String> results) {
        ...
        resolveManagerSelection(managerSelection)
                .thenAccept(newVip::setPersonalManager);
        return newVip;
    }
    ...
    public void updateEntity(VipClient entity, Map<String, String> results) {
        ...
        resolveManagerSelection(results.get("manager"))
                .thenAccept(entity::setPersonalManager);
    }
}